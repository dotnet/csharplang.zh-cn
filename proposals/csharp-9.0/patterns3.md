...
- https://github.com/dotnet/csharplang/issues/1350用圆括号模式强制或强调新组合器的优先级
- https://github.com/dotnet/csharplang/issues/1350需要两个 `and` 不同模式来匹配的联合模式;
...
- https://github.com/dotnet/csharplang/issues/812需要输入值小于、小于或等于的关系模式，等等。
...
一个微妙的实现问题是，此语法是不明确的。  诸如的字符串 `a.b` 可以作为限定名称（在类型上下文中）或点表达式（在表达式上下文中）进行分析。  编译器已经能够处理与点式表达式相同的限定名，以便处理类似的内容 `e is Color.Red` 。  编译器的语义分析将进一步扩展，以便能够将（语法）常量模式（例如，点表达式）作为一个类型进行绑定，以便将其视为绑定类型模式，以便支持此构造。
...
模式*组合器*允许使用匹配两个不同模式的两种模式 `and` （可以通过重复使用将这种模式扩展为任意数量的模式 `and` ）、使用（ditto）的两种不同模式 `or` 或使用的*求反*模式 `not` 。
...
此示例说明， `and` 解析优先级比更高（如绑定更紧密） `or` 。  程序员可以使用*带括号的模式*来使优先显式：
...
是否为 `and` 、 `or` 和 `not` 某种上下文关键字？  如果是这样，则存在重大更改（例如，在*声明模式*中将其用作指示符）。

### <a name="semantics-eg-type-for-relational-operators"></a>关系运算符的语义（如类型）
...
4. 如果 `P` 是常量模式，其中的常量不是 null 常量，并且表达式没有到*输入类型*的*常量表达式转换*，则*缩小的类型*是常量的类型。
5. 如果 `P` 是一个关系模式，其中的常量表达式没有到*输入类型*的*常量表达式转换*，则*缩小*后的类型是常量的类型。
6. 如果 `P` 是 `or` 模式，则如果存在此类通用类型，则*缩小的类型*是 subpatterns 的*缩小类型*的通用类型。 出于此目的，通用类型算法只考虑标识、装箱和隐式引用转换，并考虑一系列模式的所有 subpatterns `or` （忽略圆括号模式）。
7. 如果 `P` 是 `and` 模式，则缩小后的*类型*为适当模式的*缩小类型*。 而且，左侧模式的*缩小类型*是正确模式的*输入类型*。
8. 否则，的*缩小类型* `P` 为 `P` 的输入类型。
...
目前这不起作用，因为对于*is 模式表达式*，只会将模式变量视为*明确赋值*（在*is 模式表达式*为 true 的情况下）。

支持此方法比同时添加对求反条件语句的支持更简单（从程序员的角度来看） `if` 。  即使我们添加了此类支持，程序员也可能想知道上述代码片段不起作用。  另一方面，在中，这种情况并不 `switch` 太合理，因为*当 false*会有意义时，程序中没有相应的点可明确赋值。  是否允许在模式表达式中使用它 *，* 但在允许模式的其他上下文中不允许这样做？  这似乎是不规则的。
...
我们是否应选择推迟此类工作，直到以后（我建议如此），我们可以在 c # 9 中说
...
这种情况永远不会匹配（因为输入不能同时为 `int` 和 `double` ）。  我们检测到不能匹配的情况时，我们已遇到错误，但其措辞（"switch case 已被上一事例处理" 和 "该模式已由 switch expression 的先前版本处理"）可能会对新方案产生误导。  我们可能必须修改措辞，只是说该模式将永远不会与输入匹配。
...
诸如这样的 switch 表达式应被视为*详尽*的（它处理所有可能的输入值）。

在 c # 8.0 中，如果具有类型的输入的 switch 表达式 `byte` 包含其模式匹配所有内容的最终 arm （*丢弃模式*或*var 模式*），则它仅被视为穷举。  即使是在 c # 8 中，对每个非重复值具有 arm 的 switch 表达式也 `byte` 不会被视为详尽。  为了正确地处理关系模式的 exhaustiveness，我们还需要处理这种情况。  从技术上讲，这会是一项重大更改，但用户不可能注意到。
